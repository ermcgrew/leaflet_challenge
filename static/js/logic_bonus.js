async function main() {
    ...
    for (let i=0;i<plateGeos.length;i++) {
        plateMarkers.push(
            L.polyline(plateGeos[i].geometry.coordinates,
                {
                    color: "yellow", 
                    weight: 4,
                    fillColor: "yellow",
                    fillOpacity: 0
                }).bindPopup(`${plateGeos[i].properties.PlateName}`)

            )
        };


    //add arrays to layer groups
    let quakes = L.layerGroup(quakeMarkers);
    let platesGroup = L.layerGroup(plateMarkers);
    ...
    // Create map object
      const myMap = L.map("map", {
        ...
        zoom: 4, 
        layers: [street, quakes, platesGroup]
    });
